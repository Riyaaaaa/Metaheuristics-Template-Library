...
template< class _Individual,int _NumberOfIndividual,class _Selector,class _Scaler,bool is_Multi>
class _GA_Solver{
public:
    ...
    static constexpr double PROBABILITY_OF_MUTATION=0.1;
...
template<typename _Individual,int _NumberOfIndividual,class _Selector,class _Scaler,bool is_Multi>
_GA_Solver<_Individual,_NumberOfIndividual,_Selector,_Scaler,is_Multi>::_GA_Solver(std::vector<_Individual*> solve_targets):
...
template<class _Individual,int _NumberOfIndividual,class _Selector,class _Scaler,bool is_Multi>
void _GA_Solver<_Individual,_NumberOfIndividual,_Selector,_Scaler,is_Multi>::populationSettings(){
...
class _GA_Solver<_Individual,_NumberOfIndividual,_Selector,_Scaler,true>{
public:
    _GA_Solver(std::vector<_Individual*> solve_targets);
    _Individual* solveAnswer(int,std::string,std::string);
...
_GA_Solver<_Individual,_NumberOfIndividual,_Selector,_Scaler,true>::_GA_Solver(std::vector<_Individual*> solve_targets):
_population(solve_targets),
_mt(std::mt19937(_rnd())),
_distribution(std::uniform_real_distribution<double>(0,1))
{
...
template<class _Individual,int _NumberOfIndividual,class _Selector,class _Scaler>
_Individual* _GA_Solver<_Individual,_NumberOfIndividual,_Selector,_Scaler,true>::solveAnswer(int max_age,std::string cross_str,std::string mutation_str){
    _Individual* answer;
    std::vector<_Individual*> new_poplation;
    ...
    for(int i=0;i<max_age;i++){
        ...
        while(new_poplation.size() < _NumberOfIndividual){
            ...
            *mother = _selector(std::move(_population));
            ...
            new_poplation.push_back(father->cross_over[cross_str](mother));
            
            if(_distribution(_mt) < PROBABILITY_OF_MUTATION){
                new_poplation.push_back(_population.back()->mutation[mutation_str]());
            }
            
            new_poplation.back()->setEvalution(new_poplation.back()->calcEvalution(_aux));
            
            /*
             if(new_poplation.end() == std::find_if(new_poplation.begin(),new_poplation.end(),[&](_Individual* rhs){
                return new_poplation.back()->getEvalution() == rhs->getEvalution();
                }
             )
             )
...
template<class _Individual,int _NumberOfIndividual,class _Selector,class _Scaler>
void _GA_Solver<_Individual,_NumberOfIndividual,_Selector,_Scaler,true>::populationSettings(){
    for(_Individual* individual : _population){
        individual->setEvalution(individual->calcEvalution(_aux));
    }
    std::sort(_population.begin(), _population.end(), [](_Individual* lhs,_Individual* rhs){return lhs->getEvalution() > rhs->getEvalution();}); //descending sort
